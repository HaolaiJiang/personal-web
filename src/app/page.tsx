    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <section className="mb-20 pt-10">
        <h1 className="text-5xl md:text-7xl font-extrabold mb-6 tracking-tight text-foreground">
          Influencing the <span className="text-primary">world</span>, <br />
          like a mustard seed.
        </h1>
        <p className="text-xl md:text-2xl text-foreground/70 max-w-2xl mb-10 leading-relaxed">
          Jesus Follower • Data Scientist • AI Enthusiast • Stock Trader
        </p>
        ...
        <div className="space-y-8">
          {recentPosts.length > 0 ? (
            recentPosts.map((post) => (
              <article key={post.id} className="bg-secondary/40 rounded-none border-b border-primary/20 pb-8 hover:bg-secondary/60 transition-colors group">
                <div className="flex flex-col md:flex-row justify-between items-baseline mb-3">
                  ...
                  <span className="text-sm text-foreground/40 font-mono">{post.date}</span>
                </div>
                <p className="text-foreground/70 mb-4 text-lg">
                  {post.excerpt || post.description}
                </p>
                <Link href={`/blog/${post.slug}`} className="text-primary hover:text-accent font-medium inline-flex items-center">
                  Read full article <span className="ml-2">&rarr;</span>
                </Link>
              </article>
            ))
          ) : (
            <p className="text-foreground/60">No recent posts found.</p>
          )}
        </div>
      </section>
    </div>
